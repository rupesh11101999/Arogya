    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if User.objects.filter(email=form.cleaned_data.get('email')).count() > 0:
                ...
                return render(request,"layouts/message.html",{"background":"bg-success","title":"Verify your email","head":"Verify your email","body":"A confirmation mail has been sent to the registered email. Confirm it once to proceed."})
        else:
            errors = form.errors
    ...
    if request.user.is_client:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(request,"layouts/message.html",{"background":"bg-success","title":"Appointment Submitted","head":"Appointment Submitted","body":"Wait for appointment to be confirmed."})
